...
def download_most_recent_file(product_id, local_file_path):
    ...
    if most_recent_file:
        ...
        print(
            f'Most recent file "{most_recent_file}" downloaded successfully to {local_file_path}'
        )
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Download the most recent file with a specific prefix from an FTP site."
    )
    parser.add_argument(
        "product_id", type=str, help="The identifier for the product to download."
    )
    parser.add_argument(
        "local_file_path", type=str, help="The path to save the file locally."
    )
